...
@for ($i = 0; $i < count($producten); $i++)
	...
	<p>
		Categorie: <strong>{{ $categorie[$i] }} / {{ $subcategorie[$i] }}</strong><br>
	    Product: <strong>{{ $producten[$i]->naam }} {{ $producten[$i]->afmetingen }}</strong><br>
	    Aantal m&sup2; besteld: <strong>{{ $vierkantemeters[$i] }}</strong><br>
	    ...
	    @endif
	    Uitgerekende prijs: <strong>{{ $prijzen[$i] }}</strong><br>
    </p>
